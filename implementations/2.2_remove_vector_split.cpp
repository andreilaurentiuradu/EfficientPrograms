...
int main(int argc, char* argv[]) {
    ...
    while (getline(file1, line)) {
        ...
        file1Data[line.substr(0, commaPos)].push_back(line.substr(commaPos + 1));
    }
    file1.close();
    ...
    while (getline(file2, line)) {
        size_t commaPos = line.find(',');
        file2Data[line.substr(0, commaPos)].push_back(line.substr(commaPos + 1));
    }
    file2.close();
    ...
    while (getline(file3, line)) {
        size_t commaPos = line.find(',');
        file3Data[line.substr(0, commaPos)].push_back(line.substr(commaPos + 1));
    }
    file3.close();
    ...
    while (getline(file4, line)) {
        size_t commaPos = line.find(',');
        file4Data[line.substr(0, commaPos)].push_back(line.substr(commaPos + 1));
    }
    file4.close();
